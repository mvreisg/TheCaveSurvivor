...
namespace TheCaveSurvivor.Scene{
	 ...
	 * Classe: FirstStage
	 * 
	 * 	Gerencia a primeira fase
	 * 
	 * */

	public class FirstStage : Stages {

		protected override void Awake(){
			player = GameObject.FindObjectOfType<Player> ();
			scenePlayer = GameObject.FindObjectOfType<GameCamera>().GetComponent<AudioSource> ();			
			defaultSpawnPosition = new Vector2 (-3.1f, -1.34f);
			sceneClips = new AudioClip[] {
				Resources.Load<AudioClip>("Audio/Soundtrack/stage1_area"),
		...
		protected override IEnumerator CourseEnd(){
			...
			parabensText.text = "Ainda nao e o fim!";
			...
			minutes = Mathf.FloorToInt (GameData.gameData.FirstStageTime / 60).ToString ("00");
			seconds = Mathf.FloorToInt (GameData.gameData.FirstStageTime % 60).ToString("00");
