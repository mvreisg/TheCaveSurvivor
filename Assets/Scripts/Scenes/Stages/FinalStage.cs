...
namespace TheCaveSurvivor.Scene{
	 ...
	 * Classe: FinalStage
	 * 
	 * 	Gerencia a ultima fase
	 * 
	 * */

	public class FinalStage : Stages {

		protected override void Awake(){
			player = GameObject.FindObjectOfType<Player> ();
			scenePlayer = GameObject.FindObjectOfType<GameCamera>().GetComponent<AudioSource> ();			
			defaultSpawnPosition = new Vector2 (-8.25f, -5.63f);
			sceneClips = new AudioClip[] {
				Resources.Load<AudioClip>("Audio/Soundtrack/stage3_area"),
				Resources.Load<AudioClip> ("Audio/Soundtrack/stage3_boss"),
				Resources.Load<AudioClip> ("Audio/Soundtrack/stage3_bossdef"),
		...
		protected override IEnumerator PauseGame(){			
			if (!pausedGame) {
				...
				StartCoroutine (AudioController.GetClass.StartAudio (scenePlayer, sceneClips [5], 1f, false));
				Time.timeScale = 0;
				...
				if (Input.GetKeyDown (KeyCode.Escape)) {
					...
					Destroy (buttonsText.gameObject);
					Destroy (blackImg.gameObject);
					StartCoroutine (AudioController.GetClass.StartAudio (scenePlayer, sceneClips [6], 1f, false));
		...
		protected override IEnumerator CourseEnd(){
			...
			//StartCoroutine (AudioController.GetClass.StartAudio (scenePlayer, sceneClips [3], 0.5f, false));

			GameData.gameData.RespawnPosition = Vector2.zero;				
			GameData.gameData.PlayerLifes = player.PlayerLifes;
			GameData.gameData.PlayerPickedPickaxe = player.PickedPickaxe;
			GameData.gameData.FinalStageTime += stageTime;
			...
			parabensText.text = "E chegamos ao fim!";
			...
			minutes = Mathf.FloorToInt (GameData.gameData.FinalStageTime / 60).ToString ("00");
			seconds = Mathf.FloorToInt (GameData.gameData.FinalStageTime % 60).ToString("00");
			...
			continueText.text = "Creditos...(Enter)";
		...
		protected override void GoToNextStage(){
			...
			stopTime = false;
			enterEnd = false;
			StartCoroutine (AudioController.GetClass.StartAudio (scenePlayer, sceneClips [4], 0.5f, false));
